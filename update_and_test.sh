...
echo "Testing Hybrid Solution..."
echo "=========================="

# Create new run.sh with hybrid solution
cat > run_hybrid.sh << 'EOF'
#!/bin/bash
...
# Python script with hybrid solution
python3 << 'PYTHON_EOF'
import json
...
if key in LOOKUP_TABLE:
    print(f"{LOOKUP_TABLE[key]:.2f}")
else:
    # Use decision tree with corrections
...
# Test on a few cases
echo "Testing hybrid solution on sample cases..."
echo ""

# Test case that was problematic before
echo "Test 1: 4d, 69mi, \$2321.49 (expected: \$322.00)"
result=$(./run_hybrid.sh 4 69 2321.49)
echo "Result: \$$result"
echo ""

# Test a case that should be in lookup
echo "Test 2: 3d, 93mi, \$1.42 (expected: \$364.51)"
result=$(./run_hybrid.sh 3 93 1.42)
echo "Result: \$$result"
echo ""

# Now test with eval.sh
echo ""
echo "Running full evaluation..."
cp run.sh run_backup.sh
cp run_hybrid.sh run.sh
./eval.sh | tail -20