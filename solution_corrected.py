...
def calculate_reimbursement(trip_duration_days, miles_traveled, total_receipts_amount):
    ...
    # Decision tree logic (same as before)
    ...
    # CRITICAL CORRECTION: Apply penalty for high receipt cases
    # Analysis showed these are overestimated by 2-4x
    if total_receipts_amount > 1500:
        # Strong correction for very high receipts
        if total_receipts_amount > 2000:
            total *= 0.25  # ~4x correction
        elif total_receipts_amount > 1800:
            total *= 0.35  # ~3x correction
        else:
            total *= 0.45  # ~2x correction
    ...
    # Additional corrections for specific patterns
    if trip_duration_days <= 5 and total_receipts_amount > 1000:
        # Short trips with high receipts need more correction
        total *= 0.8
    
    if trip_duration_days >= 8 and total_receipts_amount > 1400:
